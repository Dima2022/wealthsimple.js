...
class Api {
  ...
  accessTokenInfo(accessToken = null) {
    ...
    return this.get('/oauth/v2/token/info', {
      headers: { Authorization: `Bearer ${token}`, 'X-WS-Profile': this.getCurrentProfile ? this.getCurrentProfile() : 'invest' },
  ...
  authenticate(attributes) {
    ...
    return this.post('/oauth/v2/token', { headers, body, checkAuthRefresh })
      .then((response) => {
  ...
  _fetch(method, path, {
    ...
    const executePrimaryRequest = () => {
      ...
      if (!headers['X-WS-Profile']) {
        headers['X-WS-Profile'] = this.getCurrentProfile();
      }
